...
export function recommendWeeklyGoals(historicalData: HistoricalWeek[]): RecommendedGoals {
  ...
  // Detect trend
  const isImproving = (metric: keyof Omit<HistoricalWeek, 'week'>) => {
  ...
  const getGrowthFactor = (metric: keyof Omit<HistoricalWeek, 'week'>) => {
...
export function validateGoal(
  ...
  const recentAvg = historicalData.slice(-2).reduce((sum, w) => sum + w[goalType], 0) / 2;
