...
const tutorialSteps = [
    ...
    },
    {
        icon: Target,
        title: "모험을 완료하세요",
        description: "루나가 지구에서 겪는 다양한 상황을 도와주세요. 각 모험을 완료하면 별과 경험치를 획득할 수 있어요!",
        emoji: "🎯",
    ...
    },
    {
        ...
        description: "모험과 어휘 학습으로 별을 모으고, 상점에서 파워업과 특별 아이템을 구매할 수 있어요.",
        emoji: "⭐",
    ...
    },
    {
        icon: Trophy,
        title: "주간 목표를 달성하세요",
        description: "매주 XP, 모험, 학습 시간, 단어 목표를 설정하고 달성해보세요. 100% 달성 시 특별 보상이 기다립니다!",
        emoji: "🏆",
...
export default function OnboardingTutorial({ isOpen, onComplete, onSkip }: OnboardingTutorialProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={() => { }}>
            ...
            <DialogContent
                className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-0 shadow-lg duration-200 sm:rounded-lg"
                        ...
                        <div className="flex items-center gap-3 w-full">
                            {currentStep > 0 && (
                                <button
                                    ...
                                    className="flex-1 py-4 text-base border-2 border-gray-200 rounded-xl font-bold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                                >
                                    <ChevronLeft size={20} />
                                    이전
                                </button>
                            ...
                            <button
                                onClick={handleNext}
                                className={`flex-1 py-4 text-base bg-gradient-to-r ${step.gradient} hover:opacity-90 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 transition-transform active:scale-95`}
                            >
                                {isLastStep ? "시작하기" : "다음"}
